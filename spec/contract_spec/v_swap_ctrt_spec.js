...
describe('Test class VSwapCtrt',function() {
    ...
    describe('Test method addLiquidity', function () {
        it('should add liquidity to the pool', async function () {
            ...
            const addLiqTxInfo = await this.vc.addLiquidity(this.acnt0,DELTA,DELTA,DELTA_MIN,DELTA_MIN,tenSec);
            await this.waitForBlock();
            ...
            expect((tokARes.amount.toNumber()).toBe(tokAResOld.amount.toNumber() + DELTA));
            expect((tokBRes.amount.toNumber()).toBe(tokBResOld.amount.toNumber() + DELTA));
            expect((liqTokLeft.amount.toNumber()).toBe(liqTokLeftOld.amount.toNumber() - DELTA));
        });
    });
    ...
    describe('Test method removeLiquidity', function () {
        it('should remove liquidity from the pool', async function () {
            ...
            const removeLiqTxinfo = await this.vc.removeLiquidity(this.acnt0,DELTA,DELTA,DELTA,tenSec);
            await this.waitForBlock();
            ...
            const tokARedeemed = tokAResOld.amount.toNumber() -tokARes.amount.toNumber();
            const tokBRedeemed = tokBResOld.amount.toNumber() -tokBRes.amount.toNumber();
    ...
    describe('Test method swapBForExactA', function (){
        it('should swap token B for fix amount of token A', async function () {
            ...
            const respInfo = await this.vc.swapBForExactA(acnt1,amountA,amountBMax,tenSec);
            await this.waitForBlock();
            ...
            expect(balBOld.amount.toNumber() - balB.amount.toNumber()).toBeLessThanOrEqual(amountBMax);
        });
    });
    ...
    describe('Test method swapExactBForA', function (){
        it('should swap fix amount of token B for token A', async function () {
            ...
            const respInfo = await this.vc.swapExactBForA(acnt1,amountAMin,amountB,tenSec);
            await this.waitForBlock();
            const swapTxId = respInfo.id;
            await this.assertTxSuccess(swapTxId);
            ...
            expect(balA.amount.toNumber() - balAOld.amount.toNumber()).toBeGreaterThanOrEqual(amountA);
    ...
    describe('Test method swapAForExactB', function (){
        it('should swap token A for fix amount of token B', async function () {
            ...
            const respInfo = await this.vc.swapExactBForA(acnt1,amountAMax,amountB,tenSec);
            await this.waitForBlock();
            const swapTxId = respInfo.id;
            await this.assertTxSuccess(swapTxId);
            ...
            expect(balAOld.amount.toNumber() - balA.amount.toNumber()).toBeLessThanOrEqual(amountAMax);
            expect(balB.amount.toNumber()).toBe(balBOld.amount.toNumber() + amountB);
        });
    });
    ...
    describe('Test method swapExactAForB', function (){
        it('should swap fix amount of token A for token B', async function () {
            ...
            const respInfo = await this.vc.swapExactAForB(acnt1,amountA,amountBMin,tenSec);
            await this.waitForBlock();
            const swapTxId = respInfo.id;
            await this.assertTxSuccess(swapTxId);
            ...
            expect(balA.amount.toNumber()).toBe(balAOld.amount.toNumber() - amountA);
            expect(balB.amount.toNumber() - balBOld.amount.toNumber()).toBeGreaterThanOrEqual(amountBMin);
        });
    });
